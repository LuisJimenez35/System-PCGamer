...
def Login():
    ...
    Label(ventana_login,bg="Light sky blue",text="Por favor digite usuario").pack()
    entradausuario = Entry(ventana_login, textvariable=verificaUsuario)
    entradausuario.pack()
    Label(ventana_login,bg="Light sky blue",text="Por favor digite Clave").pack()
...
def Registro():
    ...
    ventana_registro.config(bg="Light blue")
    ...
    rolcombo = StringVar()
    Label(ventana_registro,bg="Light sky blue",text="Por favor digite usuario").pack()
    entradausuarioRegistro = Entry(ventana_registro,textvariable=usuarioRegistro).pack()
    Label(ventana_registro,bg="Light sky blue",text="Por favor digite Contraseña").pack()
    entradaclaveRegistro = Entry(ventana_registro,textvariable=claveRegistro).pack()
    Label(ventana_registro,bg="Light sky blue",text="Por favor elija el rol").pack()
...
def verifica_login():
    ...
    for clave,valor in usuario.items():
        ...
        if verificaUsuario.get() in clave and verificaclave.get() in valor:
            ...
            sesionrol = valor[1]
            encontro = True
            if rolcombo.get() == "Moderador":
                roott = Toplevel(root)
            
                roott.resizable (0,0)
                roott.config(bg="Light blue")
                roott.title("Verficacion de moderador")
                clave = "supersistema"
                Label(roott,bg="Light sky blue", text="Ingrese el codigo de moderador").grid(column=0,row=0)
                btm = Entry(roott)
...
def verifica():
    if btm.get() == clave:
        ...
        messagebox.showerror("","Codigo Incorrecto")
#Ventana
def ventanaPanelAdminstrador(dato1,dato2):
    ventananueva = Tk()
    ventananueva.resizable (0,0)
    ventananueva.config(bg="Light blue")
    ventananueva.title("Ventana de Moderacion")
    Label(ventananueva,bg="Light sky blue",text="Bievenido").pack()
    if dato2 == "Moderador":
        Label(ventananueva,bg="Light sky blue",text="Bienvenido Moderador").pack()
...
files = Frame(root)
files.pack()
bg = PhotoImage( file =r"C:\Users\LuisM\OneDrive\Escritorio\WhatsApp-Image-2022-05-02-at-1.10.10-PM.png" ) 
label1 = Label( root, image = bg) 
label1.pack(ipadx= 100,ipady= 0)  
...
def APB():
    ...
    frameb = Frame(newWindow1)
    frameb.grid(column=3 , row=5)
    framec = Frame(newWindow1)
    framec.grid(column=2, row=3)
    Label(newWindow1,bg="Light sky blue",text ="Computadoras Armadas:").grid(column=3 , row=0) 
    for computador in Computadoras:
        ck=ttk.Checkbutton(framec,text=computador,variable=controlVars[(computador) + str(Recorredor)],onvalue=1,offvalue=0).pack()
    Button(newWindow1,text="Click para calcular el costo de su producto",command=calculo).grid(column=2,row=115)
...
def ubicacion(): 
    ...
    Label(newWindow2,bg="Light blue",text ="Contacto y Ubicacion").pack() 
    Label(newWindow2,bg="Light blue", text="").pack()
    Label(newWindow2,bg="Light blue", text="www.").pack()
    Label(newWindow2,bgb="Light blue", text="506 ***********").pack()
...
def componentes(): 
    ...
    newWindow4.config(bg="Light blue")
    frame1 = Frame(newWindow4,bg="#add8e6")
    frame1.grid(row=0,column=0)
    frame2 = Frame(newWindow4,bg="#add8e6")
    frame2.grid(row=0,column=1)
    frame3 = Frame(newWindow4,bg="#add8e6")
    frame3.grid(row=0,column=2)
    frame4 = Frame(newWindow4,bg="#add8e6")
    frame4.grid(row=0,column=3)
    frame5 = Frame(newWindow4,bg="#add8e6")
    frame5.grid(row=1,column=0)
    frame6 = Frame(newWindow4,bg="#add8e6")
    frame6.grid(row=1,column=1)
    frame7 = Frame(newWindow4,bg="#add8e6")
    frame7.grid(row=1,column=2)
    frame8 = Frame(newWindow4,bg="#add8e6")
    frame8.grid(row=1,column=3)
    

    Label(frame1,bg="Light sky blue",text="Procesador").pack()
    ...
    Label(frame2,bg="Light sky blue",text="Targetas madre").pack()
    ...
    Label(frame3,bg="Light sky blue",text="Tarjetas RAM").pack()
    ...
    Label(frame5,bg="Light sky blue",text="Tarjetas de video").pack()
    for grafica in GPUs:
        ttk.Checkbutton(frame5,text=grafica,variable=controlVars3[(grafica) + str(Recorredor3)],onvalue=1,offvalue=0).pack()
    
    Label(frame4,bg="Light sky blue",text="Almacenameinto").pack()
    ...
    Label(frame6,bg="Light sky blue",text="Fuentes de Poder").pack()
    ...
    Label(frame7,bg="Light sky blue",text="Cases/Gabinetes").pack()
    for caja in cases:
...
def calculo():
    ...
    for b in controlVars8:
        if controlVars8[b].get() == 1:
            ...
            total += valor8
    Label(newWindow6,bg="Light blue",text=("Total = " + str(total))).pack()
...
def lema(): 
    ...
    newWindow8.resizable (0,0)
    newWindow8.title("Quienes Somos") 
    newWindow8.geometry("500x300") 
    newWindow8.config(bg="Light blue")
    Label(newWindow8,bg="Light blue",text ="Nuestro lema").pack() 
    Label(newWindow8,bg="Light blue",text="Esta aplicacion es con fines ejemplicativos de calcular los\n precios de los componentes actuales en el mercado gaming del mundo.").pack()
    Label(newWindow8,bg="Light blue",text="Creadores: \n Luis Mora Jimenez. \n Bryan Mora Delgado. \n Alejandro Murillo Fallas.").pack()
    Label(newWindow8,bg="Light blue", text="").pack()
   
def registrodeInventario():
    global newWindow11
    newWindow11 = Toplevel(root)
    newWindow11.resizable (0,0)
    newWindow11.title("Registro de inventario")
    newWindow11.geometry("500x500")
    newWindow11.config(bg="Light blue")
    Label(newWindow11,bg="Light sky blue",text="Ingrese un nuevo componente").grid(column=5,row=0)
    Label(newWindow11,bg="Light sky blue",text="Nombre del producto").grid(column=5,row=5)
    Label(newWindow11,bg="Light sky blue",text="Precio del producto").grid(column=5,row=6)
    Label(newWindow11,bg="Light sky blue",text="¿Qué tipo de componente es?").grid(column=5,row=7)
    global entrada1
    global entrada2
    global combo
    ...
    Button(newWindow11,bg="steel blue",text="Agregar producto",command=agregar).grid(column=6,row=8)
